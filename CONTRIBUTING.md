...
- `src/lib/attendance.js` — **pure** derivation & aggregation (no `chrome.*`). Presence, sessions,
  durations, lateness, status and group roll-ups all live here. Prefer adding logic here so the
  dashboard, report and popup stay thin.
...
- If you touched `attendance.js` / `storage.js`, verify the derivation still holds (durations,
  late/absent, series matrix) on a meeting with a rejoin and an absentee.
